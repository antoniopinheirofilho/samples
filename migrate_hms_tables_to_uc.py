...
# MAGIC
# MAGIC **Important:**
# MAGIC - This notebook needs to run in a cluster with that supports Unity Catalog. It needs to have access to the existing HMS tables
...
# MAGIC - inventory_query: SQL statement that generates the inventory to be migrated. The dataset returned by this SQL statement must contain the column source_catalog, source_database, source_table, target_catalog, target_database, target_table, table_type (MANAGED, EXTERNAL or VIEW), table_provider (DELTA, PARQUET, ETC), table_locationFor example:
# MAGIC
# MAGIC `select 
# MAGIC catalog as source_catalog,
# MAGIC database as source_database,
# MAGIC name as source_table,
# MAGIC "test_uc_catalog" as target_catalog,
# MAGIC database as target_database,
# MAGIC name as target_table,
# MAGIC object_type as table_type,
# MAGIC table_format as table_provider,
# MAGIC location as table_location
# MAGIC from hive_metastore.ucx.tables`
# MAGIC
# MAGIC - is_dry_run: Indicates whether the job will execute a dry run or actually migrate the tables
...
# MAGIC - log_catalog: Catalog where the logs generated by this script will be stored
# MAGIC - log_database: Database where the logs generated by this script will be stored
# MAGIC - log_table: Table where the logs generated by this script will be stored
...
log_table_name = f"{log_catalog}.{log_database}.{log_table}"
...
def migrate_external_tables(inventory_obj, batch_id, dry_run, execution_time):
  ...
  try:
    ...
    source_catalog_tables.write.format("delta").mode("append").saveAsTable(f"{log_catalog}.{log_database}.{log_table}")

  except Exception as e:
    error = str(e).replace("'", "\\'")
    print(f"ERROR: Unable to update the log table {log_catalog}.{log_database}.{log_table}: {error}")
...
def migrate_views(inventory_df, view_name, batch_id, dry_run, execution_time):

  full_name_source = f"hive_metastore.{view_name}"

  aux_db_name = view_name.split(".")[0]
  aux_tb_name = view_name.split(".")[1]
  inv = inventory_df.where(col("source_catalog") == "hive_metastore").where(col("source_database") == aux_db_name).where(col("source_table") == aux_tb_name).collect()
  target_catalog = inv[0].target_catalog
...
external_tables = df_inventory_mapping.where(col("table_type") == "EXTERNAL")
display(external_tables)

# COMMAND ----------

# DBTITLE 1,Managed Table Inventory
managed_tables = df_inventory_mapping.where(col("table_type") == "MANAGED")
display(managed_tables)

# COMMAND ----------

# DBTITLE 1,Views Inventory
views = df_inventory_mapping.where(col("table_type") == "VIEW")
display(views)
...
if dry_run:
    ...
    for view_name in final_view_list:
        migrate_views(views, view_name, batch_id, dry_run, execution_time)
